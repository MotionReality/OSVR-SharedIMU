...
static HANDLE open_device(const char *path)
{
    ...
    // [MRI/Matt] Always open the device in shared mode.
    //  We're currently using HIDAPI to talk to the OSVR
    //  IMU which can support multiple readers. Using
    //  shared mode instead of exclusive is more robust
    //  in case e.g. CryEngine dies and hangs with the
    //  handle open.
